...
| 12 | No prev/next navigation between a course's lessons in the lesson player | Low | `features/lms/components/LessonView.tsx` | A learner finishing a lesson has to go back to the course page and find the next one manually — no in-flow "next lesson" link. | Small — `getLessonDetail` (or a sibling query) would need each lesson's ordering within its module/course to compute prev/next ids. |
| 13 | `profiles.points` balance isn't shown anywhere on the lesson page itself | Low | `features/lms/components/LessonView.tsx` | The dashboard shows points, but a learner completing a lesson (and earning `LESSON_COMPLETE` points) has no in-page confirmation of their running total — only the one-off "+N points earned" toast from `LessonCompleteButton`. | Small — `LessonPage` already fetches the user; one more `profiles.points` select and a prop pass to `LessonView`. |
| 14 | `WalletPanel`'s `name_en`/`name` branch is correct but unreachable | Low | `features/profile/components/WalletPanel.tsx` | `/profile` has no `LangToggle` anywhere (the same pre-existing gap the lesson player had before that specific fix), so `lang` never leaves its `"ar"` default — a real user never actually sees `coin_packages.name_en` today, even though the code correctly branches on it. | Small — same fix pattern already applied to the lesson player: split into a thin server page + a client view owning a real `useLang()`/`LangToggle`. Out of scope for the wallet task itself. |
