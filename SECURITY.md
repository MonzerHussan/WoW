...
## CRITICAL: instructor self-publication — CLOSED (migration 078, 2026-08-20)

Any signed-in user could make themselves a **live, requestable, paid instructor** with one direct PostgREST `PATCH`. `instructor_profiles` had a single UPDATE policy — `user_id = auth.uid()` on both `USING` and `WITH CHECK` — and 040's learner-request policy gated only on `ip.is_available`. Nothing anywhere required review. Setting `is_available = true` on your own row was sufficient to appear in front of learners and charge real coins.

**The fix (078):** review state (`approval_status`, `needs_review`, `review_note`) plus a `BEFORE UPDATE` trigger, `guard_instructor_profile_update()`, that refuses two client writes: any change to `approval_status`, and any `false → true` transition of `is_available` on a row that is not already `approved`. Hiding is always permitted — an instructor must never be trapped as visible. The learner-request policy was additionally rebuilt (drop + create, never a second policy — RLS policies for one command OR together) to require `ip.approval_status = 'approved'`, keeping 076's `initiated_by = 'learner'` direction and 040's `price_coins` snapshot verbatim.

**Why a trigger rather than a policy.** A policy constrains the `authenticated` role. The two registration functions — `submit_instructor_application()` and `review_instructor_application()` — are `SECURITY DEFINER`, run as the table owner, and with `relforcerowsecurity = false` bypass RLS entirely; they also *must* write the very columns being protected. A policy could not constrain them and could not distinguish them. A `BEFORE UPDATE` trigger sees every writer. This is the same reasoning 075 used in choosing to narrow a `WITH CHECK` rather than add a policy.

**The trigger is `SECURITY INVOKER`, deliberately.** A `DEFINER` trigger reports the function owner in `current_user`, which collapses the client/server distinction the guard is built on — the lesson from 026, recorded in 030. `current_user in ('authenticated','anon')` is 030's own `forbid_client_write` pattern.

**The `needs_review` flag is raised unconditionally, outside that check.** Had the flagging sat inside the `current_user` test, an edit routed through `submit_instructor_application()` would have escaped it — because `current_user` inside a `SECURITY DEFINER` function is the owner, not `authenticated` — turning the registration function into a way to edit an approved profile without ever being re-reviewed. The flag records **what** changed, not **who** changed it. `price_coins` is deliberately excluded: 040 pins the price into the assignment at request time and 074 charges the stored snapshot, so a price change cannot alter what an existing request costs.

**No INSERT policy on `instructor_profiles` exists, before or after.** A client cannot create a profile row at all; `submit_instructor_application()` is the only door — the same "no policy = no path, one verified function is the only door" shape as 027/028/030/033/074/077.

**Verified live against production, with the hole reproduced immediately before the fix and again as a negative test after it, using genuinely separate accounts and an independent database read after every step.** *Baseline:* an unapproved instructor's `PATCH` setting `is_available = true` returned **HTTP 200**, and an independent read confirmed the profile was **visible**. *After 078:* the identical `PATCH` returned **403/42501** — `"An instructor profile cannot be made visible before it is approved"` — and an independent read confirmed it was **still hidden**. Any client write to `approval_status` returns **403/42501** (`"approval_status is decided by review, not by the instructor"`). A learner requesting an unapproved instructor is refused **403/42501**. An unauthorized account calling `review_instructor_application()` is refused **403/42501** (`"Not authorized to review instructor applications"`). **Regressions, all re-run:** an approved instructor can hide and unhide (both 200, both confirmed by independent reads); editing `display_name` after approval sets `needs_review = true` **and leaves the profile visible**; editing `price_coins` leaves the flag `false`; approving grants the right to publish **without auto-publishing**, after which self-publish works and the learner's request returns **201**; rejecting sets `rejected` **and hides**; re-applying returns a rejected profile to `pending` with a clean flag; and an already-approved profile calling `submit_instructor_application()` **stays approved** — re-application is not a client-side route back into the queue. `audit_log` rows are written for both approve and reject.

**Grandfathered rows, recorded rather than silent:** the three profiles that predated review were set to `approved` by the migration. Two were live test instructors already visible to learners; letting them take the `'pending'` column default would have silently unpublished existing rows. New rows get `'pending'`.

## CRITICAL: instructor-assignment payment bypass — CLOSED (migration 075, 2026-08-15)

A direct PostgREST `PATCH` could set `instructor_assignments.status = 'accepted'` **without ever calling `accept_instructor_assignment()`** — i.e. without paying. The learner's request became "accepted" for free: no coins deducted, no `coin_transactions` row, no trace.

**How it arose — nobody's single mistake, a seam between two migrations.** 040 shipped the UPDATE policy (`with check (status = any (array['accepted','declined']))`) during a deliberately schema-only round, and was correct then: no payment path existed yet, so `accepted` moved no money. 040's own header even wrote the follow-up obligation down — *"No `forbid_client_write` trigger is needed yet either, since there is no payment path here yet to protect — added in the follow-up migration alongside the accept-and-charge function."* 074 then added `accept_instructor_assignment()` (which charges) but **never narrowed that policy**, so from 074 onward two routes reached the same end state: the function (charges) and a plain client `PATCH` (charges nothing).

**Why it went unnoticed until now:** no UI calls any of 074's three functions — `git grep accept_instructor_assignment -- "*.ts" "*.tsx"` returns zero, likewise `send_instructor_message`/`rate_instructor`. The missing UI is exactly what hid the hole: nothing exercised either route, so neither the correct path nor the bypass was ever run by a user. It was found by reading 074 while preparing to build that UI, not by any test.

**The fix (075):** the UPDATE policy's `WITH CHECK` narrows from `status in ('accepted','declined')` to `status = 'declined'`. Declining stays a plain client write — it moves no money, so a definer function there would add ceremony without security. `'accepted'` becomes unreachable from any client session, leaving `accept_instructor_assignment()` (SECURITY DEFINER, charging inside the same transaction as the status flip) as the only door — the same "no policy = no path, one verified function is the only writer" shape as 027/028/030/033 and 074's own message/rating tables. 074 itself was not modified.

**Verified live by replaying the original attack — two real separate accounts (instructor + learner), one variable changed (075), same account and same command before and after.** *Baseline, before 075:* the instructor's direct `PATCH` to `'accepted'` returned **HTTP 200**, and an independent read confirmed the damage rather than merely the write — `status=accepted`, learner balance **30 → 30 (unchanged)**, `coin_transactions` filtered to `reason='instructor_request'` **empty**. For contrast in the same run, the legitimate RPC moved the balance **30 → 20** and wrote a real `-10` row. *After 075:* the identical `PATCH` returned **403/42501** (`"new row violates row-level security policy for table \"instructor_assignments\""`), and an independent read confirmed `status` still **`pending`**, balance untouched, no new transaction. **Regressions, all re-run after the fix:** direct decline still succeeds (200, independently read back as `declined`); `accept_instructor_assignment()` still charges correctly (balance **20 → 10** with a new `coin_transactions` row whose `ref_id` matches that exact assignment); a non-participant third account is still refused **403/42501** by the RPC (`"Only the invited instructor may accept this assignment"`) and still gets a zero-row 200 on a direct `PATCH`, with the row left `pending` in both cases.

**Known consequence, deliberate and recorded rather than silently resolved:** 040's schema supports both directions (`initiated_by` `'learner' | 'instructor'`), but `accept_instructor_assignment()` only lets the *instructor* accept (`auth.uid() = instructor_id`). So for an instructor-initiated invite, the party who would logically accept is the learner — and after 075 they have no path to do so. That direction is decline-only until a product decision is made. This is **not a regression introduced by 075**: the only way a learner could accept an invite beforehand was the very `PATCH` that skipped payment, so that path was never legitimate — it *was* the hole. The open decision (build `learner_accept_instructor_invite()`, where caller == payer so `spend_coins()` applies directly, or drop the instructor-invites direction) is tracked in ROADMAP.md and TECH_DEBT.md.
