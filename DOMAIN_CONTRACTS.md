...
## 12. المكالمات الصوتية مع الوكيل (migration 036) — إفصاح إلزامي قبل البدء

المكالمة الصوتية هي **الموضع الوحيد على المنصة الذي يغادر فيه صوت
المستخدم جهازه**. كل ما عداه — تدريب النطق تحديداً — يبقى في المتصفح
ولا يُرفع (انظر SECURITY.md). لذلك يخضع هذا المسار لقاعدة إفصاح خاصة
به، ولا يجوز قراءة أي ضمان صوتي آخر في المنصة على أنه يشمله.

**النقل:** WebRTC من متصفح المستخدم إلى مزوّد الذكاء الاصطناعي (OpenAI)
مباشرة. الصوت لا يمر عبر خوادم WOW؛ دور الخادم محصور في إصدار بيانات
اعتماد قصيرة العمر.

**المزوّد ليس «منظمة» بمفهوم T3.** OpenAI مُعالِج فرعي (subprocessor) لا
جهة توظيف، وبند T3 (موافقة لكل منظمة على حدة على نطاقات DNA) **لا يغطي
هذا المسار، ولا يجوز الاستناد إليه كأنه يغطيه**. الغطاء هنا هو هذا
البند وحده: إفصاح صريح قبل البدء.

**توقيت الإفصاح ملزم:** يجب أن يظهر نص الإفصاح
(`agent.voiceCallDisclosure`) **قبل بدء أول مكالمة**، لا داخل صفحة
إعدادات ولا في سياسة منفصلة. الضغط على زر بدء المكالمة بعد عرضه هو
الموافقة.

**ما تحفظه WOW:** صف الجلسة (`agent_call_sessions`: البداية، النهاية،
المدة، النموذج، الكوينز المخصومة والمُعادة، ومعرّف المكالمة لدى
المزوّد) ونصّ المحادثة في `agent_messages` بعلامة `source='voice'`.
**لا تُحفظ أي بيانات صوتية لدى WOW في أي مرحلة.** والنص مرئي للمستخدم
نفسه كأي رسالة أخرى، اتساقاً مع T1.

**ما لا تتحكم به WOW ولا تدّعيه:** ما يحتفظ به المزوّد من الصوت الذي
يستقبله يخضع لسياسات بياناته لا لسياساتنا. **لا تُصدر المنصة أي ادّعاء
إيجابي بالنيابة عنه** — لا بأنه يحذف، ولا بأنه يحتفظ.

**تعارض مسجَّل مع T8:** الحذف النهائي (`data.hard_delete`) يمحو صفوفنا
ونصّ المحادثة، لكنه **لا يصل إلى صوت وصل المزوّد فعلاً**. أي استجابة
لطلب حذف بيانات يجب أن تصف هذا الحد بصدق بدل الإيحاء بمحوٍ شامل. هذا
حدّ حقيقي في نطاق T8، مسجَّل هنا لا مطموس.

**السقف مُفصَح لا مفروض:** المكالمة تُباع بكتلة 5 دقائق وتُنهيها
الواجهة عند السقف، لكن الخادم ليس على مسار الوسائط ولا يستطيع إيقاف
...
## 14. علاقة الدارس بالأستاذ أحادية الاتجاه بالتصميم (migration 076 — تصحيح نمذجة)

**العقد الملزم:** **الدارس يدعو الأستاذ. الأستاذ لا يدعو.** الدارس هو من
يطلب شرحًا لجزء يحتاج فيه عمقًا أكبر، ويدفع عند قبول الأستاذ. لا يوجد
اتجاه معاكس، ولم يكن يجب أن يوجد. قرار صريح من المالك (2026-08-15).

**هذا تصحيح نمذجة، لا ميزة مؤجَّلة.** `initiated_by='instructor'` كان
نموذجًا خاطئًا دخل الـschema في 040 ويناقض منطق المنتج نفسه — ليس نصفًا
غير مكتمل من تصميم ثنائي الاتجاه. لذلك **لم تُبنَ**
`learner_accept_instructor_invite()` ولن تُبنى دون قرار جديد وصريح.

**كيف نُفِّذ المنع (076):** إسقاط سياسة INSERT "instructor invites"
(كانت قائمة فعلًا — 040:117 تنشئها، وفحصها الذاتي 040:186 يشترط 4
سياسات ونجح، وفحص 075:81 يشترط 4 سياسات ونجح قبل 076 مباشرة، والعدد
الآن 3) **+ قيد `check (initiated_by = 'learner') not valid`**.

**القيد هو الحاجز الحقيقي، لا الاحتياطي.** `relforcerowsecurity=false`
على الجدول، أي أن **مالك الجدول وأي دالة `SECURITY DEFINER` تتجاوز RLS
بالكامل** — وهي نفس الفئة التي أنتجت ثغرة 075. أُثبت حيًا: إدراج
بصلاحيات المالك (RLS معطّل عليه) رُفض بـ**`23514` check_violation**
باسم `instructor_assignments_learner_initiated_only` صراحةً. أي سياسة
RLS وحدها ما كانت لتوقف هذا المسار.

**تعايش قيدين على العمود نفسه — مقصود، لا تعارض:**
- `instructor_assignments_initiated_by_check` — `IN ('instructor','learner')` (VALID، من 040)
- `instructor_assignments_learner_initiated_only` — `= 'learner'` (NOT VALID، من 076)

كلاهما يجب أن يمرّ، **والأضيق يحكم**. مسجَّل هنا صراحةً لأن قارئًا
مستقبليًا قد يرى الأول وحده ويستنتج أن الاتجاه مسموح — **ليس مسموحًا**.
القيد الأقدم مُبقًى عمدًا (حذفه تعديل مدمّر بلا مكسب). و`NOT VALID` سببه
صف اختبار واحد قديم بحالة `declined` يخالف القيد، وحذف بيانات حية لإرضاء
قيد هو ما يرفضه هذا المشروع — و`NOT VALID` يفرض القيد على كل إدراج
وتحديث جديد (مُثبَت حيًا أعلاه)، ويتخطى إعادة التحقق من القديم فقط.

**الدرس المنهجي (يستحق التسجيل بذاته):** جولة اختبار 040 **اختبرت هذا
الاتجاه وأجازته** — "دعوة المدرّب للمتعلم (الاتجاه المعاكس) → 201 ✅".
الاختبار كان صحيحًا والكود كان صحيحًا؛ **التصميم** كان خاطئًا. اختبار
القبول يتحقق من أن الكود يفعل ما صُمِّم له، **لا** من أن التصميم صحيح.
لا قدر من الاختبار في تلك الجولة كان سيكشف هذا.

## 15. لا أحد يصبح أستاذًا بنفسه (migration 078)

**العقد الملزم:** **التقديم مفتوح لكل مستخدم مسجَّل. الاعتماد بيد المالك
وحده. لا أحد ينشر نفسه قبل الاعتماد.** صفة "أستاذ" ليست إعدادًا يضبطه
المستخدم على نفسه — هي قرار مراجعة، لأن خلفها مالًا حقيقيًا: الدارس يدفع
كوينز عند قبول الأستاذ (074).

**قبل 078 كان العكس صحيحًا حرفيًا**: `PATCH` واحد على
`is_available = true` كان يكفي لأي مستخدم مسجَّل ليصبح ظاهرًا وقابلًا
للطلب بمقابل. أُثبت حيًا ثم أُغلق — التفصيل في `SECURITY.md`.

**الأبواب الثلاثة، ولا رابع:**
- `submit_instructor_application()` — الباب **الوحيد** لإنشاء ملف أستاذ
  أو تعديله. لا توجد سياسة INSERT على `instructor_profiles` إطلاقًا، لا
  قبل 078 ولا بعدها.
- `review_instructor_application()` — الباب **الوحيد** لتغيير
  `approval_status`، محروسة بـ`has_permission('users.manage')`.
- `guard_instructor_profile_update()` — حارس `BEFORE UPDATE` يرفض من أي
  جلسة عميل: أي مساس بـ`approval_status`، وأي انتقال
  `is_available: false → true` على ملف غير معتمَد.

**قواعد سلوكية ملزمة (قرارات مالك صريحة — لا تُغيَّر باجتهاد جلسة):**

1. **الاعتماد لا ينشر.** الاعتماد يمنح **حقّ** النشر؛ الظهور يبقى بيد
   الأستاذ عبر `is_available` بعده. من يعتمد لا يقرّر متى يكون الأستاذ
   جاهزًا للعمل.
2. **الإخفاء مسموح دائمًا.** أستاذ معتمَد يوقف الاستقبال متى شاء — ولا
   يمسّ ذلك اعتماده. أستاذ محبوس في حالة "ظاهر" عطل، لا حماية.
3. **الرفض ليس طريقًا مسدودًا.** الرفض يضبط `rejected` ويُخفي فورًا،
   **ويحفظ ملاحظة المراجعة ظاهرة للمتقدّم**. مسار العودة هو إعادة
   التقديم: `rejected → pending`. لا يوجد "حظر نهائي" في هذا النموذج.
4. **إعادة التقديم لا تُنزل ملفًا معتمَدًا إلى `pending` أبدًا.** ملف
   معتمَد يستدعي دالة التقديم يُعدَّل في مكانه ويبقى معتمَدًا. لو جاز
   العكس لصار بيد العميل إعادة ضبط حالته المعتمَدة — وهو ما يمنعه العقد.
5. **تعديل السعر ليس سببًا للمراجعة.** 040 تثبّت السعر داخل الطلب وقت
   إنشائه، و074 تخصم **اللقطة المخزَّنة** على الصف — فتغيير السعر لا
   يمكنه أن يغيّر تكلفة طلب قائم. توجيه قرار تجاري روتيني إلى المالك
   يجعله عنق زجاجة بلا حماية مقابلة.
6. **تعديل الاسم/النبذة/مجالات الخبرة بعد الاعتماد يرفع `needs_review`
   ولا يُخفي.** الملف يبقى ظاهرًا بانتظار المراجعة. إخفاء أستاذ بسبب
   تصحيح مطبعي في نبذته يقطع دخله لشكليّة.

**الوسم يسجّل ما تغيّر لا مَن غيّره.** رفع `needs_review` **غير مشروط**
بنوع الكاتب، عمدًا: `current_user` داخل دالة `SECURITY DEFINER` هو
المالك لا `authenticated`، فلو وُضع الرفع داخل فحص العميل لصارت دالة
التقديم بابًا لتعديل ملف معتمَد بلا وسم ولا إعادة مراجعة.

## 16. المحادثة متماثلة، والطلب ليس كذلك (migration 074 — الواجهة في الدفعة 3)

**العقد الملزم:** **في المحادثة، الدارس والأستاذ متساويان تمامًا.** نفس
القائمة، نفس الضوابط، نفس صلاحية القراءة والكتابة. لا طرف يرى أكثر ولا
طرف يكتب أكثر.

**وهذا التماثل مقصود بالضبط لأن ما قبله ليس متماثلًا:**

| المرحلة | التماثل |
|---|---|
| الطلب | **الدارس وحده** يطلب (076 — الأستاذ يُدعى ولا يدعو) |
| القبول | **الأستاذ وحده** يقبل (074 — والقبول يخصم من الدارس) |
| **المحادثة** | **الطرفان بالتساوي** |

سياسة القراءة في 074 متماثلة أصلًا
(`instructor_id = auth.uid() OR learner_id = auth.uid()`)، ودالة
`send_instructor_message()` تقبل الطرفين. **فأي واجهة تمنح أحدهما أكثر
تكون قد اخترعت تمييزًا لا تعرفه القاعدة** — وهذا ما يمنعه هذا البند.

**المحادثة بعد القبول فقط.** والمنع في الدالة لا في الواجهة: 
`send_instructor_message()` ترجع `assignment_not_accepted` مهما أرسل
العميل. إخفاء الطلبات غير المقبولة من القائمة **مجاملة للمستخدم، لا
حاجز** — والحاجز يُختبر على الدالة مباشرة.

**الكاتب الوحيد.** `instructor_messages` **بلا أي سياسة INSERT**، فحتى
مشارك شرعي في المحادثة يُرفض `INSERT` المباشر بـ**42501** (مُثبَت حيًا).
نفس شكل `agent_messages` (033): «لا سياسة = لا مسار، ودالة واحدة موثّقة
هي الباب الوحيد».

**الاسم لا المعرّف.** الطرف المقابل يُعرض باسمه من 077 —
`display_name` للأستاذ و`full_name` للدارس، والقرار داخل القاعدة. **وعند
غياب الاسم يُعرض وصف عام، ولا يُعرض المعرّف الخام أبدًا**: المعرّف لا
يفيد القارئ، ويكشف ما لا داعي لكشفه.
