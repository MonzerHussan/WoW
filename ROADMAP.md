...
**قرار منتج مفتوح ناتج عن 075 (يحتاج حسمًا قبل بناء الواجهة كاملة):**
`accept_instructor_assignment()` تسمح للأستاذ وحده بالقبول
(`auth.uid() = instructor_id`)، لكن الـschema (040) يدعم اتجاهين —
ومنهم `initiated_by='instructor'` (الأستاذ يدعو الطالب)، اللي المفروض
منطقيًا **الطالب** هو اللي يقبله. بعد 075 بقى الاتجاه ده بلا أي مسار
قبول (الدالة ترفض الطالب، والـPATCH المباشر مقفول). **مش انحدارًا من
075** — الطريقة الوحيدة لقبوله قبلها كانت نفس التجاوز اللي بلا دفع.
الخياران: بناء `learner_accept_instructor_invite()` (المنادي هو الدافع،
فـ`spend_coins()` تصلح مباشرة بلا حاجة لتضمين منطقها inline زي 074)،
أو إلغاء اتجاه الدعوة من الأستاذ.
