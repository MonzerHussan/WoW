...
**واجهة القبول/الرفض — الدفعة 1 من 3 (2026-08-19)**: أول باب فعلي إلى
دوال 074. قبل هذه الدفعة كان `git grep accept_instructor_assignment`
عبر `*.ts`/`*.tsx` يرجّع **صفرًا** — الطبقة الخلفية مبنية بلا مستدعٍ.
الآن: لوحة «طلبات واردة» للأستاذ فقط (`is_instructor` = وجود صف
`instructor_profiles`)، تعرض ملاحظة الدارس والسعر و**تحذير الخصم فوق
الأزرار**، وزرّي قبول/رفض. القبول عبر الدالة حصرًا، والرفض `PATCH`
مباشر — واللاتماثل مقصود ومأخوذ من 075: القبول يخصم فالسياسة تمنعه من
العميل، والرفض لا يحرّك مالًا. **لا سعر من العميل إطلاقًا** (السعر
لقطة على الصف، والدالة تعيد قراءته خادميًا — قاعدة 027). ولكل فشل
رسالة عربية خاصة (رصيد غير كافٍ بالأرقام الفعلية · رُدَّ عليه سلفًا ·
غير مخوَّل · غير موجود) لا رسالة عامة، ومسار الرفض يفحص «صفر صفوف
متأثرة» صراحةً لأن RLS تُرجع نجاحًا صامتًا (درس 018/015c/030).

**اسم الدارس للأستاذ — migration 077 (2026-08-19)**: كان الأستاذ لا يرى
اسم الدارس إطلاقًا (`profiles` قراءتها للمالك فقط)، وهو ما كان سيعطّل
المراسلة حتمًا. **لم تُضَف سياسة على `profiles`** — لأن RLS تُصفّي
الصفوف لا الأعمدة، فسياسة مهما ضُيّقت كانت ستسلّم الصف كاملًا بأعمدته
السبعة عشر (بريد، عمر، جنس، دور، حالة، نقاط) — نفس درس 063. و`REVOKE`
على مستوى العمود لا يصلح هنا لأنه يسري على الدور كله فيمنع المستخدم من
قراءة بياناته هو. الحل: دالة `SECURITY DEFINER` واحدة
(`get_my_assignment_counterparts()`) تُرجع **حقلين فقط** لطرفَي طلب
قائم بحالة `pending`/`accepted` — ولا تُرجع أطراف الطلبات المرفوضة لأن
لا سطح يحتاجها.

**والدالة تقرّر أي اسم يخرج، لا الواجهة**: `display_name` إن كان
المقابل أستاذًا، و`full_name` إن كان دارسًا. السبب أمني لا تجميلي —
أستاذ اختار اسم عرض يخالف اسمه الحقيقي (لقب مهني، أو إخفاء اسم عائلة)
كان اسمه الحقيقي سيخرج من القاعدة لو تركنا القرار للواجهة، وإخفاؤه في
العرض ليس حجبًا. **أُثبت حيًا**: وُضع في `full_name` للأستاذ نص مميّز،
ثم استدعى الدارس الدالة — الاستجابة الخام **لم تحتوِ الاسم الحقيقي
إطلاقًا** وأعادت `display_name` فقط. وهذا يحلّ أيضًا ازدواج مصدر الاسم:
الشخص يظهر باسم واحد في البطاقة والمحادثة معًا.

**الدالة صارت الحدّ الأمني** (`SECURITY DEFINER` تتجاوز RLS كليًا — نفس
خاصية ثغرة 075)، فاختُبرت بوصفها كذلك: طرف ثالث يستدعيها مباشرة →
`[]` (لا خطأ صامت ولا صف)؛ `anon` → **401/42501** `permission denied`؛
والتوقيع قُرئ **بعد** الإنشاء عبر `pg_get_function_result` لا من الكود،
والفحص الذاتي يرفض صراحةً أي عمود من الخمسة عشر ويتحقق أن سياسات
**القراءة** (`cmd = 'SELECT'`) على `profiles` بقيت **اثنتين** أي أن
الـmigration لم يوسّع قراءة إطلاقًا.

**تصحيح عدد (بتنبيه المالك)**: عبارة «سياسات `profiles` بقيت اثنتين»
كانت مكتوبة هنا بلا تقييد وهي مضلِّلة. العدد الفعلي على `profiles`
**ثلاث** سياسات: سياستا `SELECT` + سياسة `UPDATE` واحدة. الفحص الذاتي في
077 نفسه كان دقيقًا — يعدّ `cmd = 'SELECT'` ونصّ استثنائه يقول
«SELECT policies» — والخطأ كان في نثر ROADMAP وحده، لا في الـmigration.

**ويُسجَّل صراحةً هنا لأنه يجب أن يُكتب لا أن يُكتشف لاحقًا**: إحدى
سياستَي `SELECT` هي سياسة الطاقم (admin/super_admin عبر `users.manage`)
وهي تكشف **صف `profiles` كاملًا** — `email` و`age` و`gender` و`role`
ضمنه. هذا متوقَّع ومقصود لمسار الإدارة (شاشة `/admin/roles` تعرض البريد
فعلًا)، وليس تسريبًا؛ لكنه يعني أن 077 يحمي مسار **الطرف المقابل** فقط،
ولا يدّعي أي تضييق على مسار الطاقم. `search_path` مثبَّت على `public`. **الانحدارات**: الأستاذ يحاول قراءة
`email/points/role/age/gender/status` للدارس → `[]`؛ وحتى العمودين
المسموحين عبر قراءة جدول مباشرة → `[]` (الدالة هي المسار الوحيد فعلًا)؛
والطرف الثالث → `[]`؛ وكلٌّ ما زال يقرأ ملفه هو بلا انحدار. **وفي
الواجهة**: الاسم الحقيقي يظهر حين يوجد، وبديل نصي («دارس») حين يكون
فارغًا لا سلسلة فارغة، ولا أثر لاسم الأستاذ الحقيقي في الصفحة.

**بلاغ خارج نطاق 077، لم يُصلَح بأمر المالك**: `spend_coins` (007b)
دالة `SECURITY DEFINER` **بلا `search_path` مثبَّت** — على خلاف دوال
074 الثلاث المضبوطة. قابلة نظريًا لاختطاف `search_path`، وهي دالة
الخصم المركزية.
...
### الدفعة 2 — تسجيل الأساتذة والاعتماد (migration 078، 2026-08-20)

**الفجوة الحقيقية التي أُغلقت — ثغرة نشر ذاتي حيّة.** قبل 078 كان
لـ`instructor_profiles` سياسة UPDATE واحدة (`user_id = auth.uid()` على
الطرفين)، وسياسة طلب الدارس من 040 تشترط `ip.is_available` فقط. فأي
مستخدم مسجَّل كان يستطيع `PATCH` على صفّه بـ`is_available = true`
ويصبح **قابلًا للطلب فورًا بمقابل كوينز حقيقية، بلا أي مراجعة**.
**أُعيد إنتاجها حيًا على الإنتاج قبل الإصلاح**: PATCH → **200** وقراءة
مستقلة تُظهر الملف ظاهرًا. وبعد الإصلاح بدقائق: **403/42501**
`"An instructor profile cannot be made visible before it is approved"`
وقراءة مستقلة تؤكد أنه **ما زال مخفيًا**.

**لماذا حارس (trigger) لا سياسة**: السياسة تقيّد دور `authenticated`
وحده، بينما دالتا التسجيل `SECURITY DEFINER` تعملان بصلاحية مالك الجدول
و`relforcerowsecurity = false`، فتتجاوزان RLS كليًا — ولا بدّ لهما من
كتابة نفس الأعمدة المحميّة. `BEFORE UPDATE` trigger يرى كل كاتب.
والحارس **`SECURITY INVOKER` عمدًا**: حارس `DEFINER` يُظهر المالك في
`current_user` فينهار تمييز العميل/الخادم — درس 026 المسجَّل في 030،
ونمط `current_user in ('authenticated','anon')` مأخوذ من
`forbid_client_write` في 030 نفسها.

**التسجيل والحارس شُحنا في ترحيل واحد** بأمر المالك صراحةً: تجزئتهما
كانت ستفتح نافذة يصل فيها التقديم قبل الحارس — أي الثغرة نفسها وأمامها
استمارة.

**قرارات المنتج المثبَّتة**: الاعتماد **لا ينشر تلقائيًا** — يمنح حقّ
النشر، و`is_available` بيد الأستاذ بعده · الرفض يضبط `rejected`
و**يُخفي فورًا**، ومسار العودة هو إعادة التقديم (`rejected → pending`)
مع بقاء ملاحظة المراجعة ظاهرة للمتقدّم · **إعادة التقديم من ملف
`approved` لا تُعيده إلى `pending` أبدًا** (قيد المالك الصريح) · تعديل
**السعر لا يُعيد الملف إلى المراجعة** — 040 تثبّت السعر في الطلب وقت
إنشائه و074 تخصم اللقطة المخزَّنة، فلا سطح احتيال أصلًا · تعديل
الاسم/النبذة/المجالات بعد الاعتماد يرفع `needs_review` **مع بقاء الملف
ظاهرًا** — إخفاء أستاذ بسبب تصحيح مطبعي يقطع دخله لشكليّة.

**فخّ أُمسك أثناء البناء**: رفع `needs_review` كان سيهرب من التمييز لو
وُضع داخل فحص `current_user`، لأن `current_user` داخل دالة
`SECURITY DEFINER` هو المالك لا `authenticated` — فكانت الدالة ستصير
بابًا للتعديل بلا وسم. الرفع الآن **غير مشروط**، خارج الفحص: الوسم
يسجّل **ما تغيّر لا مَن غيّره**.

**14 اختبارًا حيًا نجحت كلها** بحسابات منفصلة حقيقية وقراءة مستقلة من
القاعدة بعد كل خطوة: التقديم → `pending` ومخفي · النشر الذاتي قبل
الاعتماد → 42501 · كتابة `approval_status` من أي عميل → 42501 · طلب
دارس لأستاذ غير معتمَد → 42501 · مراجعة من غير مخوَّل → 42501 · إخفاء
ثم إظهار من أستاذ معتمَد → يعملان · تعديل الاسم → `needs_review=true`
مع بقاء الظهور · تعديل السعر → الوسم يبقى `false` · الاعتماد → معتمَد
وغير منشور تلقائيًا، ثم النشر الذاتي يعمل، ثم طلب الدارس **201** ·
الرفض → مرفوض ومخفي · إعادة التقديم → `pending` نظيف · دالة التقديم من
ملف معتمَد → يبقى معتمَدًا · و`audit_log` يسجّل الاعتماد والرفض معًا.

**عطل حقيقي في الواجهة اكتُشف بالفحص الحيّ وحده — ولم يمسكه لا `tsc`
ولا `lint` ولا `build`.** صفحة `/admin/roles` كانت تُرجع **500**:
`"Functions cannot be passed directly to Client Components"`. كنت
مرّرت `instructorQueueSlot` كدالة `(lang) => ReactNode` من Server
Component إلى Client Component. القاعدة **موثّقة أصلًا في المستودع**
(`ProjectWorkspace.tsx:27`) وخالفتها. الإصلاح: نفس نمط `gamesSlot`
و`placementSlot` القائم — `ReactElement<{ lang: Lang }>` مع
`cloneElement` لحقن اللغة الحيّة. **هذا هو الدرس المنهجي للدفعة**:
الأخضر الثلاثي (tsc/lint/build) لا يثبت أن الصفحة تُفتح؛ حدود
Server/Client تنكسر وقت التشغيل فقط.

**تحقّق الواجهة حيًا (بعد الإصلاح)**: `/admin/roles` تعرض الطابور
بالشارتين معًا — «طلب جديد» للمتقدّم و«عُدِّل بعد الاعتماد» للملف
المعتمَد المعدَّل · مفتاح اللغة يمرّ فعلًا عبر `cloneElement` (تبدّلت
النصوص والشارات وحتى تنسيق التاريخ، وبقي محتوى المستخدم كما أُدخل) ·
`/instructors` لأستاذ معتمَد تعرض الطلبات الواردة باسم الطرف المقابل
(077 يعمل في الواجهة) والاستمارة وشارة «معتمَد» ومفتاح الاستقبال ·
**ومفتاح الاستقبال أُدير من الواجهة فعلًا**: `is_available` تحوّلت
`true → false` ثم عادت، وقراءة مستقلة تؤكد الحالتين مع بقاء
`approval_status` و`needs_review` بلا مساس · وللمتقدّم غير المعتمَد
**لا وجود لمفتاح الاستقبال إطلاقًا** (صفر `checkbox` في الصفحة) مع شارة
«قيد المراجعة» وظهور ملاحظة المراجعة.

**التحقّق كان بنيويًا وسلوكيًا لا بصريًا** — أداة لقطة الشاشة معطّلة في
هذه البيئة، فالإثبات من قراءة DOM وسجلّات الخادم وقراءات مستقلة من
القاعدة، لا من صورة. ونقرة المفتاح نُفِّذت بحدث DOM حقيقي على العنصر
(يمرّ بمعالج React كاملًا) لا بنقرة مؤشّر مُختبَرة بالإحداثيات.

### الدفعة 3 — المراسلة (2026-08-20، بلا ترحيل جديد)

**لا migration في هذه الدفعة.** 074 بنى كل شيء: جدول `instructor_messages`
بسياسة قراءة واحدة للأطراف، **وصفر سياسات INSERT**، ودالة
`send_instructor_message()` هي الكاتب الوحيد. المطلوب كان الواجهة فقط.

**التماثل هو القرار المعماري.** الدارس والأستاذ **متساويان في المحادثة**:
نفس القائمة، نفس الضوابط، نفس الصلاحيات. هذا ليس تبسيطًا — سياسة القراءة
في 074 متماثلة (`instructor_id = auth.uid() OR learner_id = auth.uid()`)
ودالة الإرسال تقبل الطرفين، فواجهة تمنح أحدهما أكثر كانت ستخترع تمييزًا
لا تعرفه القاعدة. يقابل ذلك تمامًا **عدم** التماثل في مسار الطلب، حيث
الأستاذ وحده يقبل (074) والدارس وحده يطلب (076).

**قائمة المحادثات تعكس القاعدة ولا تفرضها.** تظهر الطلبات `accepted` فقط،
لأن غيرها لا يحمل رسالة — لكن المنع الحقيقي في الدالة التي ترجع
`assignment_not_accepted` مهما طلب العميل. ولذلك اختُبر المنع **على
الدالة مباشرة**، لا على المكوّن.

**الاسم من 077 لا من معرّف.** رأس المحادثة والقائمة يعرضان اسم الطرف
المقابل: `display_name` إن كان أستاذًا، `full_name` إن كان دارسًا —
والقرار داخل القاعدة لا في الواجهة. وحين لا يوجد اسم يُعرض وصف («الطرف
الآخر») **لا المعرّف الخام**؛ المعرّف لا يفيد القارئ ويكشف شيئًا.

**الاستطلاع (polling) — قرار مبرَّر لا نمط ثانٍ.** `AgentChat` لا يستطلع
لأن الوكيل يردّ داخل نفس الطلب، فلا رسالة تصل لم يرسلها المستخدم. هنا
الطرف الآخر يكتب مستقلًا، وبلا استطلاع لا يظهر ردّه إلا بإعادة تحميل
يدوية — وهو في محادثة مدفوعة يُقرأ كعطل. المدة **10 ثوانٍ**، والمؤقّت
يُلغى عند إغلاق المحادثة أو تبديلها. (البديل الأنظف مستقبلًا: Supabase
Realtime — لم يُستخدم لأنه نمط جديد كليًا في هذا المشروع، والقرار معروض
عليك.)

**٧ اختبارات قاعدة بيانات حيّة، كلها نجحت، بثلاثة حسابات منفصلة وقراءة
مستقلة من الطرف المقابل بعد كل خطوة:**

| # | الاختبار | النتيجة |
|---|---|---|
| T1 | الدارس يرسل على طلب مقبول | `{sent:true}` · **قرأها الأستاذ** بقراءة مستقلة |
| T2 | الأستاذ يردّ | `{sent:true}` · **قرأها الدارس** بقراءة مستقلة |
| T3 | الدارس يرسل على طلب **قيد الانتظار** | `{sent:false, reason:'assignment_not_accepted'}` · وقراءة مستقلة تؤكد **صفر رسائل** (لا نجاح صامت) |
| T4a | **طرف ثالث** يقرأ المحادثة | `[]` — مصفوفة فارغة |
| T4b | **طرف ثالث** يحاول الإرسال فيها | **403/42501** `"Not a participant in this assignment"` |
| T5 | **دارس مشارك فعلًا** يحاول `INSERT` مباشرًا في `instructor_messages` | **403/42501** — الدالة هي الكاتب الوحيد فعلًا، لا مجرد المسار الذي تسلكه الواجهة |
| T6 | محتوى فارغ | `{sent:false, reason:'empty_content'}` |

T5 خارج القائمة المطلوبة، وأُضيف لأن «صفر سياسات INSERT» ادّعاء يجب أن
يُثبَت بمحاولة كتابة من **مشارك شرعي**، لا من غريب — الغريب يُرفض لأسباب
أخرى فلا يفصل في المسألة.

**تحقّق الواجهة حيًا (خادم محلي، ثلاثة حسابات):**
- **جانب الأستاذ**: محادثتان مقبولتان، الاسم «سارة الدارسة»، ودور «دارس»،
  ونص الطلب الأصلي · رسالة الدارس **يسارًا** وردّ الأستاذ **يمينًا**.
- **إرسال فعلي من الواجهة** بزرّ الإرسال → قراءة مستقلة من القاعدة تؤكد
  وصول الرسالة بـ`sender_id` الصحيح.
- **جانب الدارس (نفس المحادثة)**: **انعكاس تام** — رسالته يمينًا ورسالتا
  الأستاذ يسارًا · ورأس المحادثة **«أستاذ اختبار الواجهة»** أي
  `display_name` لا الاسم الحقيقي.
- **الاستطلاع مُثبَت**: أُرسلت رسالة من الأستاذ عبر REST وصفحة الدارس
  مفتوحة، فظهرت خلال 13 ثانية **بلا إعادة تحميل**
  (`navigation.type === "navigate"` لا `"reload"`).
- **الطرف الثالث على `/instructors`**: **لا قسم محادثات إطلاقًا** · صفر
  حقول كتابة · ولا أثر لأي رسالة ولا لأي اسم من أسماء الطرفين.
- **ولا معرّف خام (UUID) على الصفحة في أي من الحالتين** — فُحص بتعبير
  نمطي على نص الصفحة كاملًا، لا بالنظر.

**تصحيح تعليق قديم**: توثيق `getIncomingInstructorRequests` كان ما زال
يقول إن اسم الدارس **غير متاح للأستاذ** — صحيح قبل 077 وبائت بعده،
والكود تحته يعيد `learnerName` فعلًا. صُحِّح.

**التحقّق بنيوي وسلوكي لا بصري** (أداة لقطة الشاشة معطّلة في هذه البيئة).
